...
| **L1** | `blocks/l1/` | Leaf Block classes. Currently just `MmaBody` — the triple-nested MMA tile loop with multiple call shapes. Single-use helpers that used to live here (`IndexCache`, `WorkListLoad`, `QRegisterLoad`, `GatheredTileLoad`) are now free functions under `popcorn.lang`. |
...
- Pattern used in 2+ kernels **or** >15 lines with clear single
  responsibility → L1 Block **or** `pop.*` free function (prefer the
...
it, it belongs in L1 / L2 / `popcorn.lang`.
...
Kernels also import `popcorn.lang as pop` for IR ops (`pop.mul`,
`pop.for_range`, `pop.barrier`), epilogue helpers (`pop.store_acc`,
`pop.atomic_store_acc`, `pop.silu`, `pop.cast`), and the memory
helpers that replaced the L1 dataclass wrappers (`pop.work_list_load`,
`pop.index_cache`, `pop.q_register_load`). See
[ARCHITECTURE.md](ARCHITECTURE.md#authoring-surface--popcornlang) and
[IR.md](IR.md).
...
| `ScatterStore` / `SiluCastStore` | `pop.store_acc(dst, acc, row=, col=, cast=, activation=)` |
| `AtomicScatterAdd` | `pop.atomic_store_acc(dst, acc, col=, index=, weight=, op="add")` |
| `NormalizeAndStore` (attn) | `pop.store_acc(..., per_warp=True, row_scale=l_vals)` |
| `WorkListLoad` | `grp_start, expert = pop.work_list_load(work_list, work_idx)` |
| `IndexCache` | `smem_region = pop.index_cache(name, gmem, count=, base=)` |
| `QRegisterLoad` | `q_frags = pop.q_register_load(g_q, smem=, q_row=, warp_id=, MT=, Dh=, kv_pad=)` |
...
requires an explicit `Value`. In particular `pop.for_range(0, n, 1)`
...
pop.store_acc(g.Out, acc, row=m_base, col=n_base + warp_off, cast=DType.BF16)
```
...
| `(count, init_value, dtype)` | `count` copies of `pop.const(dtype, init_value)` |
...
`pop.yield_(carry)` auto-flattens a Carry argument — inside a
hand-rolled `for_range`, the idiom is `pop.yield_(carry)` rather than
`pop.yield_(*carry.flatten())`.
...
| `emit_silu_cast_epilogue(b, acc_results, g_out, …)` / `emit_atomic_scatter_epilogue(b, …)` | legacy epilogue emitters (use `pop.store_acc` / `pop.atomic_store_acc` instead) |

Most kernel authors never call these directly — the DSL methods
(`SmemTile.load_from`, `.gather_from`) and the `pop.*` free functions
...
`GatheredTileLoad`) are now free functions under `popcorn.lang` —
each was a 4-50 line emit body that didn't benefit from a class.

### Epilogues (L1-ish, live in `popcorn.lang`)

Always frag → smem → vec_store for block-sized tiles; never scalar
direct to gmem. The `pop.store_acc` fast path stages through smem
...
pop.store_acc(g.h, acc, row=token_row, col=n_base,
              cast=DType.BF16, activation="silu", stage_in_smem=True)

# MoE outproj — atomic scatter-add with row weights.
pop.atomic_store_acc(g.Out, acc, col=n_base, index=toks, weight=weights)
...
pop.store_acc(g_out, o_acc, row=out_row_warp, col=0,
...
2. `pop.async_commit()` / `pop.async_wait(0)` — iff produce emitted any `cp.async` (detected via `Builder.async_emissions` counter)
3. `barrier("block")`
4. `body.consume(ictx)` → new carry
5. `barrier("block")`
6. `pop.yield_(*new_carry)`
...
automatically so `pop.store_acc(g.Out, acc, …)` works without the
...
- **`SharedRegion` subscript / `warp_view` / `copy_from`** — unified
  ...
  [IR.md](IR.md).
- **`pop.min` / `pop.max(x, dim=)` / `pop.sum(x, dim=)`** — dim-kwarg
