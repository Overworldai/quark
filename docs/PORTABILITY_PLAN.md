...
| 3.2 | ``SpirVLowerer`` body — ~45 visitors + ``LoweredSpirVKernel`` artifact | ✅ functional — 14 / 27 in-tree kernels pass auto-parameterized smoke vs `reference_numpy`; visitor surface covers arith / cmp / select / convert / bitcast / math (GLSL.std.450) / load / store / vec_load / vec_store / vec_build / vec_extract / smem_alloc / barrier / subgroup reduce / shuffle / lane_id / subgroup_id / group_id / thread_idx / block_idx / block_dim / if/else (with carries via OpPhi) / for_loop (with carries) / yield / atomic_rmw / split_b32 / merge_b32 / mul_hi (via OpUMulExtended). dtypes: F32, U32, S32, BF16 (via ``SPV_KHR_bfloat16``), F16, PRED, B16, B32. Wide vec (>4) gated as NotImplementedError — Vulkan compute disallows ``Vector16``; future work to lower as chains of width-4 ops. | wide-vec → chain-of-4 lowering for the 16-byte bf16 cohort |
