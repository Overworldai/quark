...
For every problem:
  * ``ptx  (µs)``: ``pcf.gemm`` with ``QUARK_DISABLE_CUBLAS=1`` so
  ...
  * ``cublas (µs)``: direct ``dispatch_cublas`` call — includes the
    ``pcf.quantize_e4m3`` cast when the spec is mixed-dtype.
...
def _bench_ptx(p: Problem, A, B, Out, *, n_warmup: int, n_bench: int) -> float:
    ...
    # ``pcf.gemm`` is forced onto the PTX kernel — which is what we
