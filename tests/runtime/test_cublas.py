...
Strategy: for each supported combo, run ``qf.gemm`` twice — once with
...
def _run(A, B, out_dtype, *, disable_cublas: bool):
    """Run ``qf.gemm`` with cuBLAS toggled via env var."""
    with _env("QUARK_DISABLE_CUBLAS", "1" if disable_cublas else None):
        return qf.gemm(A, B, out_dtype=out_dtype)
...
class TestDispatchGate:
    ...
    def test_mixed_bf16_e4m3_falls_through(self, monkeypatch):
        ...
        calls = _patch_record_cublas_calls(monkeypatch)
        with _env("QUARK_DISABLE_CUBLAS", None):
            qf.gemm(A, B, out_dtype="bf16", compute_dtype="e4m3")
    ...
    def test_activation_falls_through(self, monkeypatch):
        ...
        calls = _patch_record_cublas_calls(monkeypatch)
        with _env("QUARK_DISABLE_CUBLAS", None):
            qf.gemm(A, B, out_dtype="bf16", activation="silu")
    ...
    def test_b_shuffled_falls_through(self, monkeypatch):
        ...
        with _env("QUARK_DISABLE_CUBLAS", None):
            try:
                qf.gemm(A, B, out_dtype="bf16", b_shuffled=True)
            except Exception:
    ...
    def test_bias_uses_cublas(self, monkeypatch):
        ...
        calls = _patch_record_cublas_calls(monkeypatch)
        with _env("QUARK_DISABLE_CUBLAS", None):
            qf.gemm(A, B, out_dtype="bf16", bias=bias)
        assert len(calls) == 1
