    ...
    for p in problems:
        ...
        for target in (None, DType.F32):
            ...
            if spec_dtypes and not (spec_dtypes <= _SPV_SUPPORTED_DTYPES):
                continue
            # Resolve empty ``config.main_shape`` from device caps —
            # the Launcher does this internally during compile, but
            # the pre-validation ``is_valid_for`` below sees the
            # un-resolved config, so we mirror the resolver here.
            # Without this, MMA-using kernels with ``main_shape=""``
            # default would fall through ``lookup_mma`` to a PTX
            # shape and fail the pre-validation check.
            resolved_cfg = _LAUNCHER._maybe_resolve_main_shape(
                kernel_cls, k.spec, k.config,
            )
            cand = (
                k if resolved_cfg is k.config
                else kernel_cls(k.spec, resolved_cfg)
            )
