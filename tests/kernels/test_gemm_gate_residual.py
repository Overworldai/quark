...
dispatch, replacing the standalone ``pcf.gemm`` followed by
``pcf.ada_gate_residual``. This test verifies the fused output
...
def test_fused_gate_residual_matches_reference(M, N, K, G):
    ...
    out = pcf.gemm(A, B, gate=Gate, residual=Residual, gate_groups=G, out_dtype="bf16")
    # Explicit drain: ``pcf.gemm`` enqueues the dispatch through
