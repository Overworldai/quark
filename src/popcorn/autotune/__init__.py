...
``popcorn.autotune.cache``; disk I/O + JSON helpers live in
``popcorn.autotune.io``; the full genetic search body lives in
``popcorn.autotune.full_search``. This package's ``__init__`` just
hosts:

  * ``_SEARCH_DEPTH`` / ``current_search_depth()`` — fast vs full toggle.
  * ``_verbose()`` / ``_log()`` — single source of truth for diagnostic
    output gated on ``POPCORN_AUTOTUNE_VERBOSE``.
...
# ``from popcorn.autotune import AutotuneCache``. Import at the bottom to
...
_VERBOSE_ENV = "POPCORN_AUTOTUNE_VERBOSE"
...
from popcorn.autotune.cache import AutotuneCache  # noqa: E402
