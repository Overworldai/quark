...
from popcorn.ir import Function
from popcorn.ir.lifetime import Lifetime, LifetimeKind
from popcorn.ir.op import (
    SmemAllocOp,
)
from popcorn.ir.region import Region
...
def _value_use_indices(fn: Function, op_index: dict[int, int]) -> dict[int, list[int]]:
    ...
    Three classes of uses are recorded:
      ...
      * Same for ``src_tensor`` / ``dst_tensor`` on LoadMatrixOp /
        StoreMatrixOp.
    """
    from popcorn.ir.tensor import SharedRegion
