"""MSL lowerer for the popcorn IR.
...
from popcorn.ir.tensor import Tensor
...
class MslLowerer:
    ...
    def lower_function(self, fn: Function, module: Module | None = None) -> LoweredMslKernel:
        ...
        return LoweredMslKernel(
            source=source,
            header=header,
            kernel_name=f"popcorn_{fn.name}",
    ...
    def _emit_smem_allocs(self, fn: Function, ctx: _MslCtx) -> None:
        ...
        from popcorn.lower.smem_layout import compute_smem_layout, dump_smem_layout

        plan = compute_smem_layout(fn, enable_aliasing=self._smem_aliasing_enabled)
        if _os.environ.get("POPCORN_DUMP_SMEM_LAYOUT", "").lower() in ("1", "true", "yes", "on"):
    ...
    def _hoist_consts(self, ops: list[Op], ctx: _MslCtx) -> None:
        ...
        """
        from popcorn.ir import ConstOp as _ConstOp
