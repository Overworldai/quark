"""popcorn.lang — kernel authoring surface.

EXEMPT FROM 500-LINE RULE: this module is the single ``import popcorn.lang
as pop`` namespace kernel authors hit; every free function here is a
...
Usage:
    ...
    with pop.kernel_scope(bld):
        x = pop.load(ptr, idx)
        y = pop.exp2(x)
        pop.store(out, idx, y)

        with pop.for_range(pop.const(DType.S32, 0),
                           pop.const(DType.S32, K),
                           pop.const(DType.S32, 1)) as (k, _):
            ...
...
from popcorn.lang.epilogue import (
    atomic_store_acc,
    cast,
    silu,
    store_acc,
...
from popcorn.lang.memory import (
    index_cache,
    q_register_load,
    work_list_load,
)
...
def current_builder() -> Builder:
    ...
    Reads ``popcorn.ir.value._ACTIVE_BUILDER`` which is published by
    ...
    if bld is None:
        raise RuntimeError(
            "popcorn.lang: no active Builder. Call this from inside a "
            "kernel emit body (between begin_function / end_function), "
            "or wrap a bare block in `with popcorn.lang.kernel_scope(bld):`."
        )
    return bld
...
def min(*args, **kwargs):
    """``pop.min(a, b)`` — elementwise min.

    ``pop.min(frag, dim=1)`` — cross-lane min reduction along the MMA
...
def max(*args, **kwargs):
    """``pop.max(a, b)`` — elementwise max.

    ``pop.max(frag, dim=1)`` — per-row max reduction across the MMA
...
def sum(*args, **kwargs):
    """``pop.sum(frag, dim=1)`` — per-row sum reduction across the
    MMA frag. No ``pop.sum(a, b)`` form; use ``a + b`` for addition."""
    if "dim" in kwargs:
        return _frag_reduce_dim("add", args[0], kwargs["dim"])
    raise TypeError(
        "pop.sum: requires a `dim=` kwarg (cross-lane reduction). "
        "Use ``a + b`` for scalar addition."
    )


def _frag_reduce_dim(kind: str, value, dim: int):
    """Shared dim→axis router for pop.{min,max,sum}(x, dim=).
    ...
    """
    from popcorn.blocks.dsl import active_bctx

    if dim not in (0, 1):
        raise ValueError(f"pop.{kind}: dim must be 0 or 1, got {dim}")
    axis = "row" if dim == 1 else "col"
    bctx = active_bctx()
    cfg = bctx.mma_cfg
    if cfg is None:
        raise RuntimeError(
            f"pop.{kind}(x, dim={dim}): no active MMA shape on bctx. "
...
# `with pop.for_range(...)` / `with pop.if_(...)` just forward.
def for_range(*args, **kwargs):
    """Python-friendly wrapper over ``Builder.for_loop``.

    Lifts Python ``int`` bounds (``lo`` / ``hi`` / ``step``) to
    :class:`~popcorn.ir.Value` constants using U32 as the default dtype
    so kernel authors can write::

        with pop.for_range(0, n_chunks, 1, iv_name="k", carried=carry):
...
def yield_(*args, **kwargs):
    # Auto-flatten a Carry argument: ``pop.yield_(carry)`` is equivalent
    # to ``pop.yield_(*carry.flatten())`` — the loop ``carried=`` API
    # still wants a flat tuple of Values under the hood.
    from popcorn.blocks.dsl import Carry
