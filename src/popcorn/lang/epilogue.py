...
functions in ``popcorn.blocks.epilogue`` so IR emission stays
...
from popcorn.blocks.dsl import Accumulators, BlockContext, active_bctx
...
def silu(values: list[Value]) -> list[Value]:
    ...
    if not values:
        return []
    from popcorn.lang import const, ex2_approx, neg, rcp_approx, vec_build
...
def cast(values: list[Value], dtype: DType) -> list[Value]:
    ...
    if values[0].dtype == dtype:
        return list(values)
    from popcorn.lang import convert
...
def atomic_store_acc(
    ...
    *,
    ...
    from popcorn.blocks.l0.epilogue import emit_atomic_scatter_epilogue
...
def _auto_alloc_staging(
    ...
    *,
    ...
    """
    from popcorn.lang import smem_alloc
...
def _silu_scalar(elem: Value, log2e: Value, one: Value) -> Value:
    ...
    from popcorn.lang import ex2_approx, neg, rcp_approx
