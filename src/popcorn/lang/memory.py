...
from popcorn.ir import DType, GlobalTensor, SharedRegion, Value
...
def work_list_load(work_list: GlobalTensor, work_idx: Value) -> tuple[Value, Value]:
    ...
    """
    bctx = active_bctx()
    wl_base = pop.mul(work_idx, bctx.c(2))
    grp_start = pop.bitcast(pop.load(work_list, wl_base), DType.U32)
    expert = pop.bitcast(pop.load(work_list, pop.add(wl_base, bctx.c(1))), DType.U32)
...
def q_register_load(
    ...
    *,
    ...
    of width-N b32 Values ready to feed ``pop.mma`` (or the Triton-
...
def index_cache(
    ...
    *,
    ...
    Callers should issue ``pop.barrier("block")`` before reading the
    cached values (the cooperative load stores to smem but the
    downstream readers may be on different threads).
    """
    from popcorn.blocks.l0.index_cache import emit_index_cache

    bctx = active_bctx()
    dt = dtype or gmem.dtype
    smem = pop.smem_alloc(name, dt, (count,))
