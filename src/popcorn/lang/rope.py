...
def emit_rope_cos_sin(
    ...
    *,
    ...
    w_f = pop.convert(w_idx, DType.F32)
    h_f = pop.convert(h_idx, DType.F32)
    t_f = pop.convert(frame_t, DType.F32)
    ...
    with pop.if_(is_x, carried=[zero_f]) as (then_in, else_in, arms):
        with arms.then_():
            # X band: local_i = c_idx / 2, base = pi * (1 + local_i * scale)
            half_c = pop.convert(c_idx >> bctx.c(1, dtype=DType.U32), DType.F32)
            x_base = pi_c + half_c * xy_scale
            x_freq = x_pos * x_base
            pop.yield_(x_freq)
        with arms.else_():
            # Not x-band: determine if y or t.
            is_y = pop.cmp("lt", c_idx, c_xy_dim)
            with pop.if_(is_y, carried=[zero_f]) as (y_then, y_else, y_arms):
                with y_arms.then_():
                    # Y band: local_i = (c_idx - x_dim) / 2
                    y_local = pop.convert(
                        (c_idx - c_x_dim) >> bctx.c(1, dtype=DType.U32), DType.F32
                    )
                ...
                with y_arms.else_():
                    ...
                    t_base = pop.ex2_approx(pop.neg(t_exp * log2_theta))
                    t_freq = t_f * t_base
                    pop.yield_(t_freq)
            (inner_freq,) = pop.last_results()
            pop.yield_(inner_freq)
    (freq,) = pop.last_results()

    # cos/sin via SFU.
    cos_val = pop.cos(freq)
    sin_val = pop.sin(freq)
