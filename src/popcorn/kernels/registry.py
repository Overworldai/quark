...
Per popcorn cleanup proposal §4.4. The registry is the single
authority on what kernels exist; tools (`tools/bench.py`,
`tools/fuzz.py`, `tools/autotune.py`) read from it instead of
maintaining hand-rolled `TARGETS` dicts.

Goal: adding a new kernel touches one place — the kernel folder
itself. The auto-discovery loop in `popcorn/kernels/__init__.py`
...
# Note: the proposal also requires `make_tensors`, but it coexists
# with a legacy instance-method version in `Kernel` (called by
# tools/bench.py / tools/autotune.py). We can't reliably compare
# function identity across the two shapes (instance method vs
# classmethod), so make_tensors is *not* validated by the function-
# identity check above. Tests at smoke / fuzz time will surface a
# missing override the first time they call it.
    ...
    def wrap(cls: type) -> type:
        ...
        if missing:
            raise TypeError(
                ...
                f"See popcorn cleanup proposal §4.3 for the contract."
            )
