...
from popcorn.kernels.ada_gate_residual.baselines import ada_gate_residual_baselines
from popcorn.kernels.ada_gate_residual.config import AdaGateResidualConfig
from popcorn.kernels.ada_gate_residual.problems import ada_gate_residual_problems
from popcorn.kernels.ada_gate_residual.reference import ada_gate_residual_reference_for_spec
from popcorn.kernels.ada_gate_residual.spec import AdaGateResidualSpec
...
@kernel(
    ...
    reference=ada_gate_residual_reference_for_spec,
)
class AdaGateResidualKernel(Kernel):
    ...
    def make_tensors(cls, problem: dict) -> dict:
        ...
        Y = PT.astype(PT.randn(spec.B, spec.D), dt)
        gate = PT.astype(PT.randn(spec.G, spec.D) * 0.5, dt)
        Out = PT.zeros(spec.B, spec.D, dtype=dt)
        return {"X": X, "Y": Y, "gate": gate, "Out": Out}
    ...
    def build(self) -> None:
        ...
        # ── Smem ──
        G_smem = pop.smem_alloc("G_smem", dtype, (D,), pad=0)
        n_stages = min(2, n_chunks)
        stages = [
            _GateStage(
                X=pop.smem_alloc(f"X_s{i}", dtype, (n_warps, chunk_D), pad=0),
                Y=pop.smem_alloc(f"Y_s{i}", dtype, (n_warps, chunk_D), pad=0),
            )
        ...
        def consume(ictx: IterCtx) -> tuple:
            ...
            for v in range(vecs_per_lane):
                ...
                g_vec = pop.vec_load(G_smem, col_off + vc, width=vec_elems, dtype=dtype)
                x_vec = pop.vec_load(stage.X, bctx.warp_id, vc, width=vec_elems, dtype=dtype)
                y_vec = pop.vec_load(stage.Y, bctx.warp_id, vc, width=vec_elems, dtype=dtype)

                out_elems = []
                for j in range(vec_elems):
                    gf = pop.convert(pop.vec_extract(g_vec, j), DType.F32)
                    xf = pop.convert(pop.vec_extract(x_vec, j), DType.F32)
                    yf = pop.convert(pop.vec_extract(y_vec, j), DType.F32)
                    out_elems.append(pop.convert(pop.fma(gf, yf, xf), dtype))
