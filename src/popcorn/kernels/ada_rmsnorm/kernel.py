...
class AdaRMSNormKernel(Kernel):
    ...
    def build(self) -> None:
        ...
        # ── Smem ──
        S_smem = pop.smem_alloc("S_smem", dtype, (D,), pad=0)
        B_smem = pop.smem_alloc("B_smem", dtype, (D,), pad=0)
        n_stages = min(2, n_chunks)
        x_stages = [
            _XStage(X=pop.smem_alloc(f"X_s{i}", dtype, (n_warps, chunk_D), pad=0))
        ...
        def consume_p1(ictx: IterCtx) -> tuple:
            ...
            for v in range(vecs_per_lane):
                ...
                for j in range(vec_elems):
                    x = pop.convert(pop.vec_extract(x_vec, j), DType.F32)
                    local_sum = pop.fma(x, x, local_sum)
            return (local_sum,)
        ...
        (final_sum,) = p1_result
        total = pop.subgroup_reduce("sum", final_sum)
        ...
        def consume_p2(ictx: IterCtx) -> tuple:
            ...
            for v in range(vecs_per_lane):
                vc = (lane * vecs_per_lane + v) * vec_elems
                x_vec = pop.vec_load(stage.X, bctx.warp_id, vc, width=vec_elems, dtype=dtype)
                s_vec = pop.vec_load(S_smem, col_off + vc, width=vec_elems, dtype=dtype)
                b_vec = pop.vec_load(B_smem, col_off + vc, width=vec_elems, dtype=dtype)

                out_elems = []
                for j in range(vec_elems):
                    x = pop.convert(pop.vec_extract(x_vec, j), DType.F32)
                    sc = pop.convert(pop.vec_extract(s_vec, j), DType.F32)
                    bi = pop.convert(pop.vec_extract(b_vec, j), DType.F32)
                    y = pop.fma(x * rms_inv, one_f + sc, bi)
                    if do_silu:
                        assert log2e is not None
                        sig = pop.rcp_approx(one_f + pop.ex2_approx(pop.neg(y * log2e)))
                        y = y * sig
