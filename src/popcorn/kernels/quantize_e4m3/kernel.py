...
class QuantizeE4M3Kernel(Kernel):
    ...
    def build(self) -> None:
        ...
        for i in range(epl):
            ...
            val_lo = pop.convert(g.X[src_idx_lo], DType.F32)
            val_hi = pop.convert(g.X[src_idx_hi], DType.F32)

            # packed_convert: two f32 → one b16 (two packed e4m3 bytes).
            # PTX: cvt.rn.satfinite.e4m3x2.f32 d, a, b
            packed = pop.packed_convert(val_lo, val_hi, DType.E4M3)
            ...
            dst_idx = pair_idx * c2
            pop.store(g.Out, packed, dst_idx)
