...
class RMSNormKernel(Kernel):
    ...
    def _build_large_d(self, D, n_warps, n_threads, dtype, epl, vec_elems, min_chunk, lane, warp_c):
        ...
        from popcorn.blocks import PipelineBody
        ...
        x_stages = [
            _Stage(X=pop.smem_alloc(f"X_s{i}", dtype, (n_warps, chunk_D), pad=0))
    ...
    def _build_small_d(self, D, n_warps, n_threads, dtype, epl, vec_elems, lane, warp_c):
        ...
        flat_smem = pop.smem_alloc("X_flat", dtype, (rows_per_load, D), pad=0)

        block_row_base = pop.block_idx("y") * bctx.c(rows_per_load, dtype=DType.U32)
        ...
        for ri in range(rows_per_warp):
            ...
            for e in range(epl):
                ...
                x = pop.convert(flat_smem[smem_row, col], DType.F32)
            ...
            for e in range(epl):
                col = bctx.c(e) * warp_c + lane
                y = x_regs[e] * rms_inv
                g.Out[gmem_out_row, col] = pop.convert(y, dtype) if dtype is not DType.F32 else y
