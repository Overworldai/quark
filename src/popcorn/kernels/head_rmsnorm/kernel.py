...
from popcorn.kernels.head_rmsnorm.baselines import head_rmsnorm_baselines
from popcorn.kernels.head_rmsnorm.config import HeadRMSNormConfig
from popcorn.kernels.head_rmsnorm.problems import head_rmsnorm_problems
from popcorn.kernels.head_rmsnorm.reference import head_rmsnorm_reference_for_spec
from popcorn.kernels.head_rmsnorm.spec import HeadRMSNormSpec
...
@kernel(
    ...
    reference=head_rmsnorm_reference_for_spec,
)
class HeadRMSNormKernel(Kernel):
    ...
    def make_tensors(cls, problem: dict) -> dict:
        ...
        X = PT.astype(PT.randn(spec.M, spec.D_full), dt)
    ...
    def build(self) -> None:
        ...
        flat_smem = pop.smem_alloc("X_flat", dtype, (heads_per_load, Dh), pad=0)

        # Block-level head base.
        block_head_base = pop.block_idx("y") * bctx.c(heads_per_load, dtype=DType.U32)
        ...
        for hi in range(heads_per_warp):
            ...
            needs_norm = pop.cmp("lt", abs_head_in_qkv, n_norm_c)
            ...
            for e in range(epl):
                ...
                x = pop.convert(flat_smem[local_head, col], DType.F32)
                local_sum_sq = pop.fma(x, x, local_sum_sq)
                x_regs.append(x)

            total = pop.subgroup_reduce("sum", local_sum_sq)
            one_f = bctx.c(1.0, dtype=DType.F32)
            rms_inv = pop.rsqrt_approx(
                total * bctx.c(1.0 / Dh, dtype=DType.F32) + bctx.c(s.eps, dtype=DType.F32)
            )
            rms_inv = pop.select(needs_norm, rms_inv, one_f)
            ...
            for e in range(epl):
                ...
                g.Out[gmem_out_token, gmem_out_col_base + col] = (
                    pop.convert(y, dtype) if dtype is not DType.F32 else y
                )
