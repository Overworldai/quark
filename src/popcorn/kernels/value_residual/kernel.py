...
class ValueResidualKernel(Kernel):
    ...
    def build(self) -> None:
        ...
        for i in range(epl):
            ...
            v = pop.convert(g.V[off], DType.F32)
            v1 = pop.convert(g.V1[off], DType.F32)
            # out = v + lamb * (v1 - v)
            out = pop.fma(lamb_f, v1 - v, v)
            g.Out[off] = pop.convert(out, dtype) if dtype is not DType.F32 else out
