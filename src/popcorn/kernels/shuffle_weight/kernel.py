...
from popcorn.kernels.shuffle_weight.config import ShuffleWeightConfig
from popcorn.kernels.shuffle_weight.spec import ShuffleWeightSpec
...
class ShuffleWeightKernel(Kernel):
    ...
    def build(self) -> None:
        ...
        # raw byte indices and uses u8 loads/stores via pop.store with
        ...
        for i in range(bpl):
            ...
            is_in_range = pop.cmp("lt", d, c_perm_coverage)
            ...
            val = g.Src[src_elem]
            pop.store(g.Dst, val, dst_elem, pred=is_in_range)
