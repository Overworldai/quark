"""Numpy conversion helpers for popcorn dtypes.
...
def to_f32_numpy(x: Any, *, dtype_hint: str | None = None) -> np.ndarray:
    ...
    Inputs:
      ...
      * ``PopcornTensor`` — copied to host via ``to_bytes()`` +
    ...
    if (
        ...
        raise TypeError(f"to_f32_numpy: unsupported PopcornTensor dtype {dt!r}")
...
def astype_numpy(arr_f32: np.ndarray, target) -> np.ndarray:
    ...
    ``target`` may be a ``popcorn.ir.DType`` enum, a short string
    ...
    The carrier convention mirrors ``PopcornTensor.from_numpy`` — pass
