"""Dispatch layer between PopcornTensor ops and popcorn kernels.

EXEMPT FROM 500-LINE RULE: every PopcornTensor op routes through here.
...
PopcornTensor.__add__ etc. call functions here, which build the
appropriate spec, compile via the Launcher, and launch. All kernels
go through the standard popcorn pipeline (IR → lowerer → driver).
...
if TYPE_CHECKING:
    from popcorn.launcher.launcher import CompiledKernel
...
def _broadcast_rows(t: PopcornTensor, n_rows: int) -> PopcornTensor:
    """Expand [1, D] → [n_rows, D] by repeating the single row."""
    from popcorn.runtime.tensor import PopcornTensor

    D = t.shape[1]
    parts = [t] * n_rows
    return PopcornTensor.cat(parts, dim=0).reshape(n_rows, D)


def elemwise_binary(
    op: str, a: PopcornTensor, b, *, out: PopcornTensor | None = None
) -> PopcornTensor:
    """Element-wise binary op. ``b`` can be a PopcornTensor or scalar.
...
def elemwise_binary_scalar_lhs(op: str, scalar: float, t: PopcornTensor) -> PopcornTensor:
    """scalar <op> tensor."""
    return _binary_with_scalar_lhs(op, float(scalar), t)


def _binary_tensors(
    op: str, a: PopcornTensor, b: PopcornTensor, *, out: PopcornTensor | None = None
) -> PopcornTensor:
    """Binary op on two same-shape tensors."""
    from popcorn.ir import DType
    from popcorn.kernels.elementwise.config import ElementwiseConfig
    from popcorn.kernels.elementwise.spec import ElementwiseSpec
    from popcorn.runtime.tensor import PopcornTensor
    ...
    # pure overhead.
    Out = PopcornTensor.empty(padded_N, dtype=orig_dtype)
...
def _binary_with_scalar(op: str, t: PopcornTensor, scalar: float) -> PopcornTensor:
    ...
    compiled = _launcher().compile(_elemwise_cls(), spec, config)
    Out = PopcornTensor.zeros(padded_N, dtype=orig_dtype)
...
def _binary_with_scalar_lhs(op: str, scalar: float, t: PopcornTensor) -> PopcornTensor:
...
def elemwise_unary(op: str, t: PopcornTensor) -> PopcornTensor:
    ...
    dummy_Y = PopcornTensor.zeros(1, dtype=orig_dtype)
...
def cast(
    ...
    target_dtype: str,
    *,
    out: PopcornTensor | None = None,
    dummy_y: PopcornTensor | None = None,
) -> PopcornTensor:
    ...
    the per-call ``PopcornTensor.empty`` — used by ``nn.Cast`` to
    ...
    y = dummy_y if dummy_y is not None else PopcornTensor.zeros(1, dtype=t.dtype)
    ...
    if N % epb == 0:
        ...
        spec = probe_spec
        compiled = _launcher().compile(_elemwise_cls(), spec, config)
        Out = out if out is not None else PopcornTensor.empty(*orig_shape, dtype=target_dtype)
    ...
    Out_padded = PopcornTensor.empty(padded_N, dtype=target_dtype)
...
def scalar_increment(t: PopcornTensor) -> None:
    ...
    from popcorn.ir import DType
    from popcorn.kernels import get
    from popcorn.kernels.increment.kernel import IncrementConfig, IncrementSpec
...
def copy_strided(t: PopcornTensor) -> PopcornTensor:
    """Copy a non-contiguous tensor into a fresh contiguous buffer."""
    from popcorn.runtime.tensor import PC_BYTES, PopcornTensor, _contiguous_strides

    N = t.numel()
    if N == 0:
        return PopcornTensor.empty(*t.shape, dtype=t.dtype)

    elem = PC_BYTES[t.dtype]

    # Fast path: contiguous data at an offset → simple D2D memcpy.
    from popcorn.runtime.tensor import _is_contiguous

    if _is_contiguous(t.shape, t.strides) and t._offset > 0:
        out = PopcornTensor.empty(*t.shape, dtype=t.dtype)
        from popcorn.runtime.cuda import CudaRuntime
    ...
    from popcorn.ir import DType
    from popcorn.kernels.copy_strided.config import CopyStridedConfig
    from popcorn.kernels.copy_strided.spec import CopyStridedSpec
    ...
    # We create a "fake" PopcornTensor that points to the full storage.

    src_full = PopcornTensor(
    ...
    out = PopcornTensor.zeros(padded_N, dtype=t.dtype)
...
def copy_strided_into(dst: PopcornTensor, src: PopcornTensor) -> None:
...
def fill_scalar(t: PopcornTensor, value: float) -> None:
...
def reduce_op(op: str, t: PopcornTensor, dim: int | None) -> PopcornTensor:
    ...
    if dim is None:
        ...
        out = PopcornTensor.from_bytes(out_buf.tobytes(), (1,), "f32")
    ...
    out = PopcornTensor.from_bytes(out_list.tobytes(), out_shape, "f32")
    return cast(out, t.dtype) if t.dtype != "f32" else out
...
def matmul(a: PopcornTensor, b: PopcornTensor) -> PopcornTensor:
...
def cat(tensors: list[PopcornTensor], dim: int = 0) -> PopcornTensor:
    ...
    from popcorn.runtime.tensor import PC_BYTES, PopcornTensor
    ...
    out = PopcornTensor.empty(*out_shape, dtype=dtype)
...
def randn(shape: tuple[int, ...], dtype: str) -> PopcornTensor:
    ...
    t = PopcornTensor.from_bytes(buf.tobytes(), shape, "f32")
    if dtype != "f32":
        t = cast(t, dtype)
    return t
...
def _pad_tensor(t: PopcornTensor, padded_N: int) -> PopcornTensor:
...
def _fill_tensor(n: int, value: float, dtype: str) -> PopcornTensor:
    ...
    if value == 0.0:
        return PopcornTensor.zeros(n, dtype=dtype)

    t = PopcornTensor.empty(n, dtype=dtype)
    from popcorn.runtime.cuda import CudaRuntime
