...
keep the per-config dance portable between CUDA (PopcornTensor) and
Metal (mx.array) without reaching into ``popcorn.backend.PT`` — which
...
def zero_buffer(buf: Any) -> Any:
    ...
    PopcornTensor → in-place memset. mx.array → returns a new
...
def ir_dtype_of(buf: Any) -> DType:
    """Resolve ``buf.dtype`` to a popcorn ``DType`` enum.

    PopcornTensor stores dtype as a short string; mx.array carries an
    ...
    if isinstance(dt, str):
        ir = _PC_TO_IR.get(dt)
        if ir is None:
            raise TypeError(f"ir_dtype_of: PopcornTensor dtype {dt!r} has no popcorn.ir.DType")
        return ir
