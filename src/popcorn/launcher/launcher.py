...
Per popcorn launcher proposal §6.1 + §6.2.
...
from popcorn.device import Device, DeviceFamily, current_device
...
def _check_dtype_mlx(buf, expected: DType) -> None:
    ...
    actual = table.get(buf.dtype)
    if actual is None:
        raise TypeError(f"_check_dtype_mlx: mlx dtype {buf.dtype} has no popcorn IR equivalent")
    if actual is not expected:
        raise TypeError(
...
class CompiledKernel:
    ...
    def launch(
        ...
        *,
        ...
        from popcorn.device import DeviceFamily
...
def _driver_for(family: DeviceFamily):
    ...
    if family is DeviceFamily.CUDA:
        from popcorn.drivers.cuda import CudaDriver
...
class Launcher:
    ...
    def __init__(
        ...
        # in popcorn.autotune for callers that always pass explicit
    ...
    def compile(
        ...
        ``POPCORN_DISABLE_AUTOTUNE=1`` to skip the search and fall
    ...
    def _lower(self, ir_or_program, kernel):
        ...
        raise NotImplementedError(
            ...
            "return an `popcorn.ir.Module` from emit()."
        )
