...
from popcorn.ir import DType, SharedRegion, Value
...
def emit_shuffled_bfrag_load(
    ...
    *,
    ...
    Returns:
      A width-``frag_regs`` b32 Value, ready to pass to ``pop.mma()`` as B.
    """
    ...
    if frag_regs == 1:
        return pop.load(b_smem_lane, row_c, col_c, dtype=DType.B32)
    return pop.vec_load(
        b_smem_lane,
        row_c,
        col_c,
