...
class Accumulators:
    ...
    on ``self.results`` so epilogue helpers (``pop.store_acc``,
    ``pop.atomic_store_acc``) can take just the ``Accumulators``
    ...
    def init(self) -> list[Value]:
        ...
        """
        zero = pop.const(self.dtype, self.fill_value)
        return [pop.vec_build([zero] * self.width) for _ in range(self.MT * self.NT)]
