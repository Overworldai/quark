...
from popcorn.blocks.dsl.context import active_bctx
from popcorn.ir import Builder, DType, SharedRegion
...
class SmemTile:
    ...
    ``pop.smem_alloc`` happens automatically in ``__post_init__`` via
    ...
    def _allocate(self, bld: Builder) -> None:
        ...
        self._smem = pop.smem_alloc(self.name, self.dtype, self.shape, pad=self.pad)
