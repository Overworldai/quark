...
from popcorn.blocks.dsl.context import _ACTIVE_BCTX
from popcorn.ir import Builder, DType, Value

if TYPE_CHECKING:
    from popcorn.kernels.gemm.mma_shapes import MmaConfig
...
class BlockContext:
    ...
    def tid(self) -> Value:
        if self._tid is None:
            self._tid = pop.thread_idx("x")
        return self._tid

    @property
    def gid(self) -> Value:
        if self._gid is None:
            self._gid = pop.group_id()
        return self._gid

    @property
    def tig(self) -> Value:
        if self._tig is None:
            self._tig = pop.thread_id_in_group()
        return self._tig

    @property
    def tig_x2(self) -> Value:
        if self._tig_x2 is None:
            self._tig_x2 = pop.mul(self.tig, self.const(DType.U32, 2))
        return self._tig_x2

    @property
    def warp_id(self) -> Value:
        if self._warp_id is None:
            self._warp_id = pop.div(self.tid, self.const(DType.U32, 32))
        return self._warp_id

    @property
    def lane_id(self) -> Value:
        return self.hoist("lane_id", lambda: pop.rem(self.tid, self.const(DType.U32, 32)))

    def hoist(self, name: str, compute_fn: Any) -> Value:
        """Compute an expression once and cache by name.

        Usage:
            val = ctx.hoist("warp_b_offset", lambda: pop.mul(warp_id, ctx.c(64)))
    ...
    def const(self, dtype: DType, value: int | float) -> Value:
        ...
        ``pop.const``) freely inside nested regions.
        """
