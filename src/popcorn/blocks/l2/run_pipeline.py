...
from popcorn.blocks.dsl import BlockContext, Carry
...
def _resolve_init_carry(body: PipelineBody, bctx: BlockContext) -> tuple[Value, ...]:
    ...
    """
    from popcorn.blocks.dsl import Accumulators, Carry
...
def _stash_results_on_carry(body: PipelineBody, final: tuple[Value, ...]) -> None:
    ...
    rebound Carry so ``pop.store_acc(o_acc, ...)`` works without the
...
def _iter_accumulators(carry: Any) -> Any:
    ...
    """
    from popcorn.blocks.dsl import Accumulators
