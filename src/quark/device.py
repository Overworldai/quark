...
class DeviceFamily(Enum):
    ...
    # the register table keys on this family.
    INTEL_GPU = "intel_gpu"
    CPU = "cpu"
...
#
# 32 is correct for every backend currently wired up (NV / Metal /
# Apple GPU / CDNA) and for the SPIR-V Intel path when SIMD32 is
# pinned via ``reqd_sub_group_size(32)``. RDNA wave64 and Intel non-
# pinned builds are the failure modes; they require the
# is_valid_for caps work first.
...
class DeviceCaps:
    ...
    # form get the butterfly expansion from the legalize pass.
