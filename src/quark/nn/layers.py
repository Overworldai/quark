...
def _cached_out_buf(module, shape, dtype):
    ...
    wraps a ``qf.*`` kernel needs to own its output buffer (same trick
    ...
    def forward(
        ...
        *,
        ...
        if gemm_bias is not None:
            y = qf.gemm(x, self.weight.data, bias=gemm_bias, out=out, **kw)
        else:
            y = qf.gemm(x, self.weight.data, out=out, **kw)
...
def _maybe_cached_out(module, shape, dtype):
    ...
    skip ``out=`` and let ``qf.*`` route through ``queue_launch_ir``.
...
class AdaRMSNorm(Module):
    ...
    def forward(self, x, scale, bias, *, activation=None):
        ...
        return qf.ada_rmsnorm(x, scale, bias, activation=activation, out=out)
...
class AdaGateResidual(Module):
    ...
    def forward(self, x, y, gate):
        ...
        out = _maybe_cached_out(self, tuple(x.shape), x.dtype)
        return qf.ada_gate_residual(x, y, gate, out=out)
...
class HeadRMSNorm(Module):
    ...
    def forward(self, qkv):
        ...
        return qf.head_rmsnorm(qkv, n_q_heads=self._nq, n_kv_heads=self._Hk, Dh=self._Dh, out=out)
...
class EulerStep(Module):
    ...
    def forward(self, x, v, dsig):
        ...
        out = _maybe_cached_out(self, tuple(x.shape), x.dtype)
        return qf.euler_step(x, v, dsig, out=out)
...
class MLP(Module):
    ...
    def forward(self, x, *, gate=None, residual=None, gate_groups: int = 1):
        ...
        # through Linear → qf.gemm → GemmKernel.build()'s
...
class ValueResidualPacked(Module):
    ...
    def forward(self, qkv_curr, qkv_first):
        ...
        return qf.value_residual_packed(
            qkv_curr,
            qkv_first,
            self.lamb.data,
...
class KVCacheUpdate(Module):
    ...
    calls ``qf.kv_cache_update`` with the right kwargs.
    """
