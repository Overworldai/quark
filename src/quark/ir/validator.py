"""Structural validation + perf-warning pass for quark IR modules.
...
them at every compile); set ``QUARK_ENABLE_PERF_WARNINGS=1`` to
opt in to the non-fatal emissions.

See QUARK_IR_PROPOSAL.md §11.
"""
...
class PerfWarning(UserWarning):
    ...
    Off by default. Set ``QUARK_ENABLE_PERF_WARNINGS=1`` in the env
...
def _perf_warnings_enabled() -> bool:
    return os.environ.get("QUARK_ENABLE_PERF_WARNINGS", "").lower() in (
        "1",
        "true",
        "yes",
        "on",
