...
Used by ``quark.functional.gemm`` to short-circuit the custom kernel
...
A × B^T" convention quark uses for weights is free alignment here.
...
# quark dtype string → cudaDataType_t
...
class CublasRuntime:
    ...
    def matmul(
        ...
        *,
        ...
        Dispatched from ``quark.functional.gemm`` after the dtype
