...
from quark.blocks.dsl import Accumulators, Block, BlockContext, active_bctx
from quark.blocks.l0.mma_tile import emit_mma_tile
from quark.blocks.l0.shuffled_bfrag_load import emit_shuffled_bfrag_load
from quark.ir import SharedRegion, Value
...
def _infer_k_inner_from_b(b: Any, mma_k: int) -> int | None:
    ...
    """
    from quark.blocks.dsl import SmemTile
        ...
        * ``mma(a=, b=, acc=)`` — Triton-style ``tl.dot`` form:
          pass the A and B operand views directly. Each may be:
            ...
            - Pre-loaded register fragments as ``list[list[Value]]``
              ...
              width-N Value fed straight into ``qk.mma`` (no smem
          ...
          input to every ``qk.mma`` in the tile loop; return value
        ...
        for inner_k in range(ki):
            ...
            for m in range(mt_count):
                for n in range(nt_count):
                    ...
                    acc[i] = qk.mma(cfg.shape_id, a_frag, b_frag, acc[i])
        return acc
