...
Two storage tiers:
  ...
  * **On-disk** — ``~/.cache/quark/refs/<hash>.npz`` so subsequent
...
a kernel, clear the cache manually (``rm -r ~/.cache/quark/refs``
or ``python -m quark.refs --clear``). Adding per-source-file hash
...
def _default_cache_dir() -> Path:
    """``~/.cache/quark/refs`` (or ``$QUARK_CACHE_DIR/refs`` when
    set). Created on first write."""
    root = os.environ.get("QUARK_CACHE_DIR")
    base = Path(root) if root else Path.home() / ".cache" / "quark"
    return base / "refs"
...
def _cli() -> int:
    ...
    parser = argparse.ArgumentParser(description="quark reference-oracle cache")
