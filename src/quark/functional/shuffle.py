...
Pre-shuffle B ahead of time so ``qf.gemm(A, B_shuf, b_shuffled=True)``
...
Use once at load time:
    ...
    from quark.functional.shuffle import shuffle_b_for_gemm
    B_shuf = shuffle_b_for_gemm(A, B)
    # store B_shuf next to B in the module; pass it to qf.gemm at call time
    C = qf.gemm(A, B_shuf, b_shuffled=True)

The shuffle layout (K_CHUNK, mma_k, b_pad) is read from the same
resolved config ``qf.gemm`` would use on this device — mismatched
...
from quark.kernels import get as _registry_get
from quark.weight_shuffle import ShuffledWeight, shuffle_b_for_frag_load
...
def shuffle_b_for_gemm(A, B, *, out_dtype=None, compute_dtype=None):
    """Offline-shuffle ``B`` so ``qf.gemm(A, B_shuf, b_shuffled=True)``
...
def shuffle_b_for_moe_inproj(X, W_in, *, n_experts, top_k=2, out_dtype=None, compute_dtype=None):
    """Offline-shuffle ``W_in`` for ``qf.moe_inproj``.
...
def shuffle_b_for_moe_outproj(
    ...
    """Offline-shuffle ``W_out`` for ``qf.moe_outproj``.
