"""``quark.functional.patchify`` — patchify kernel (strided A-tile GEMM).

    out = qf.patchify(X_flat, W, B=1, C=32, H=32, W_spatial=64)
...
def patchify_2x2(X, W):
    ...
    if X.ndim == 4:
        ...
        raise ValueError("patchify_2x2: use qf.patchify() for flat [B, C*H*W] input")
    else:
