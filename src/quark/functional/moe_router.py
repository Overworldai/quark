...
Signature:
    ...
    token_ids, slot_weights, counts = pcf.moe_router(
...
counts[E])`` ready to feed straight into ``pcf.moe_inproj`` and
``pcf.moe_outproj``. The ``work_list`` those kernels also need is
