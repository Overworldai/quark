"""``quark.functional.noise_cond`` — cached noise-conditioner LUT.
...
quark kernel calls (the MLP result is already baked in).

Setup (one-time, at model init):

    lut = precompute_noise_lut(sigmas, freqs, W1, W2)

Inference (hot path, zero non-quark ops):
...
from quark.functional.gemm import gemm as _pcf_gemm
...
def precompute_noise_lut(sigmas, freqs, W1, W2):
    ...
    pure Python (no numpy). The MLP (2x qf.gemm) runs on the GPU.
    """
    n = len(sigmas)
...
def _precompute_cuda(sigmas, freqs_list, fourier_dim, n, W1, W2):
    ...
    if out_dt in ("f16", "bf16"):
        return QuarkTensor.from_numpy(emb, dtype=out_dt)
    return QuarkTensor.from_numpy(emb, dtype="bf16")
