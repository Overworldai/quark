...
Sibling modules in the ``autotune`` package:
  ...
  * ``quark.autotune`` (this package's ``__init__``) owns the depth
    contextvar and generic GA primitives (``_crossover`` / ``_mutate`` /
    ``_parallel_compile``).
  * ``quark.autotune.io`` owns disk I/O + JSON (de)serialization.
  * ``quark.autotune.full_search`` owns the genetic loop used by
    ``qf.<op>.autotune()`` / ``quark.max_autotune()``.
"""
...
if TYPE_CHECKING:
    from quark.device import Device
...
class AutotuneCache:
    ...
    **Full search**: full genetic algorithm, same warm seeding. Triggered by
    ``qf.<op>.autotune()``, the ``QUARK_MAX_AUTOTUNE=1`` env var, or
    ``with quark.max_autotune()``.

    Owned by :class:`quark.launcher.Launcher` (one per device per
    ...
    # fired on compile failures during full search.
    ...
    def _search_full(
        ...
        *,
        ...
        ``tune_space`` overrides the kernel's default space.
        """
    ...
    def _search_full_result(
        ...
        *,
        ...
        Used by callers that want to report the winning runtime
        alongside the config (e.g. warmup APIs / offline tuners).
        """
        from quark.autotune.full_search import run_full_search
