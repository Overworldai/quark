...
Python ``random.gauss`` loop that ``QuarkTensor.randn`` previously
used.
...
from quark.kernels.randn.config import RandnConfig
from quark.kernels.randn.problems import randn_problems
from quark.kernels.randn.reference import randn_reference_numpy
from quark.kernels.randn.spec import RandnSpec
...
class RandnKernel(Kernel):
    ...
    def _philox_round(
        ...
        c0, c1, c2, c3 = ctr
        k0, k1 = key
        hi0 = qk.mul_hi(M0_c, c0)
        lo0 = qk.mul(M0_c, c0)
        hi1 = qk.mul_hi(M1_c, c2)
        lo1 = qk.mul(M1_c, c2)
        new_c0 = qk.xor(qk.xor(hi1, c1), k0)
        new_c1 = lo1
        new_c2 = qk.xor(qk.xor(hi0, c3), k1)
        new_c3 = lo0
        new_k0 = qk.add(k0, W0_c)
        new_k1 = qk.add(k1, W1_c)
    ...
    def _u32_to_uniform(self, bctx, x_u32, scale_c, bias_c):
        ...
        """
        x_shifted = qk.shr(x_u32, bctx.c(8, dtype=DType.U32))
        x_f = qk.convert(x_shifted, DType.F32)
        return qk.fma(x_f, scale_c, bias_c)
    ...
    def _box_muller(self, bctx, u1, u2, neg_2_ln2_c, two_pi_c):
        ...
        """
        log2_u1 = qk.log2_approx(u1)
        # -2*ln(u1) = log2(u1) * (-2 * ln(2)). u1 ≤ 1 → log2 ≤ 0 → arg ≥ 0.
        arg = qk.mul(log2_u1, neg_2_ln2_c)
        r = qk.sqrt_approx(arg)
        theta = qk.mul(u2, two_pi_c)
        z1 = qk.mul(r, qk.cos(theta))
        z2 = qk.mul(r, qk.sin(theta))
        return z1, z2
    ...
    def build(self) -> None:
        ...
        ctr_offset = qk.load(g.counter_offset, zero_u)
