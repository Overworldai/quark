...
class OwlAttnKernel(Kernel):
    ...
    def spec_from_tensors(
        ...
        *,
        ...
        if Q.ndim != 2:
            raise ValueError(f"qf.owl_attn: Q must be rank-2 (flat), got {Q.shape}")
        if packed_qkv:
        ...
        if quilt_factor < 1 or (quilt_factor & (quilt_factor - 1)) != 0:
            raise ValueError(
                f"qf.owl_attn: quilt_factor must be a power of 2 (>=1), got {quilt_factor}"
            )
        if quilt_offset < 0 or quilt_offset >= quilt_factor:
            raise ValueError(
                f"qf.owl_attn: quilt_offset must be in [0, quilt_factor); "
                f"got offset={quilt_offset} factor={quilt_factor}"
            )
        if tpf % quilt_factor != 0:
            raise ValueError(
                f"qf.owl_attn: H_spatial*W_spatial={tpf} must be divisible "
        ...
        if tuple(K_cache.shape) != (B * n_kv_heads * capacity, Dh):
            raise ValueError(
                f"qf.owl_attn: K_cache shape {tuple(K_cache.shape)} != "
                f"({B * n_kv_heads * capacity}, {Dh})"
            )
        if tuple(Vt_cache.shape) != (B * n_kv_heads * Dh, capacity):
            raise ValueError(
                f"qf.owl_attn: Vt_cache shape {tuple(Vt_cache.shape)} != "
                f"({B * n_kv_heads * Dh}, {capacity})"
            )
        if int(segments.shape[0]) != B * max_segments * 2:
            raise ValueError(
                f"qf.owl_attn: segments shape {tuple(segments.shape)} != ({B * max_segments * 2},)"
            )
        if int(n_segments.shape[0]) != B:
            raise ValueError(f"qf.owl_attn: n_segments shape {tuple(n_segments.shape)} != ({B},)")
