...
class KVCacheUpdateKernel(Kernel):
    ...
    def is_valid(self) -> bool:
        ...
        # ``(t_start + r) // quilt`` lines up with the per-tile compaction
        # of K_out / Vt_out.
        quilt = s.quilt_factor
        if quilt < 1 or (quilt & (quilt - 1)) != 0:
            return False  # power of 2 only
        if s.quilt_offset < 0 or s.quilt_offset >= quilt:
            return False
        if c.tile_T % quilt != 0:
            return False
        if s.tpf % quilt != 0:
            return False
        tile_T_cached = c.tile_T // quilt
    ...
    def spec_from_tensors(
        ...
        *,
        ...
        if K.ndim != 2:
            raise ValueError(f"qf.kv_cache_update: K must be rank-2 (flat), got {K.shape}")
        if packed_qkv:
        ...
        if quilt_factor < 1 or (quilt_factor & (quilt_factor - 1)) != 0:
            raise ValueError(
                f"qf.kv_cache_update: quilt_factor must be a power of 2 (>=1), got {quilt_factor}"
            )
        if quilt_offset < 0 or quilt_offset >= quilt_factor:
            raise ValueError(
                f"qf.kv_cache_update: quilt_offset must be in [0, quilt_factor); "
                f"got offset={quilt_offset} factor={quilt_factor}"
            )
        if tpf % quilt_factor != 0:
            raise ValueError(
                f"qf.kv_cache_update: H_spatial*W_spatial={tpf} must be divisible "
        ...
        if not packed_qkv and tuple(V.shape) != tuple(K.shape):
            raise ValueError(
                f"qf.kv_cache_update: V shape {tuple(V.shape)} != K shape {tuple(K.shape)}"
            )
        if tuple(K_cache.shape) != (B * n_kv_heads * capacity, Dh):
            raise ValueError(
                f"qf.kv_cache_update: K_cache shape {tuple(K_cache.shape)} != "
                f"({B * n_kv_heads * capacity}, {Dh})"
            )
        if tuple(Vt_cache.shape) != (B * n_kv_heads * Dh, capacity):
            raise ValueError(
                f"qf.kv_cache_update: Vt_cache shape {tuple(Vt_cache.shape)} != "
                f"({B * n_kv_heads * Dh}, {capacity})"
            )
        if int(segments.shape[0]) != B * max_segments * 2:
            raise ValueError(
                f"qf.kv_cache_update: segments shape {tuple(segments.shape)} != "
                f"({B * max_segments * 2},)"
            )
        if int(n_segments.shape[0]) != B:
            raise ValueError(
                f"qf.kv_cache_update: n_segments shape {tuple(n_segments.shape)} != ({B},)"
            )
        return KVCacheUpdateSpec(
            ...
            in_dtype=DType.from_backend(K),
            kv_dtype=DType.coerce(kv_dtype) or DType.from_backend(K_cache),
    ...
    def build(self) -> None:
        ...
        # Cache row math is in *cache-slot space* (compacted by quilt). Each
        ...
        # ``r_full = qoff + r_cached * quilt`` so RoPE only runs on the
        # kept pixels (1/quilt of the input rows). When quilt=1 this collapses
        ...
        # Vt_out[c, r_cached] = V_in[qoff + r_cached*quilt, c]. The input
        # row stride in cached space is quilt (1 in the dense path).
        ...
        # source values come from rows that are quilt apart in V_in.
        if is_fp8:
