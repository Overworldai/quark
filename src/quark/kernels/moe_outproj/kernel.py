    ...
    def spec_from_tensors(
        ...
        *,
        ...
        if h_in.ndim != 2 or W_out.ndim != 2:
            raise ValueError(
                f"qf.moe_outproj: h_in, W_out must be rank-2; "
                f"got h_in={h_in.shape}, W_out={W_out.shape}"
            )
        total_slots, H = int(h_in.shape[0]), int(h_in.shape[1])
        if total_slots % n_experts != 0:
            raise ValueError(
                f"qf.moe_outproj: h_in.shape[0] ({total_slots}) not divisible by "
                f"n_experts ({n_experts})"
            )
        capacity = total_slots // n_experts
        if total_slots < M * top_k:
            raise ValueError(
                f"qf.moe_outproj: h_in slot budget ({total_slots}) too small for "
                f"M*top_k ({M * top_k}); need capacity*n_experts >= M*top_k"
            )
        nW, H_w = int(W_out.shape[0]), int(W_out.shape[1])
        if H_w != H:
            raise ValueError(f"qf.moe_outproj: h_in.shape[1] ({H}) != W_out.shape[1] ({H_w})")
        if nW % n_experts != 0:
            raise ValueError(
                f"qf.moe_outproj: W_out.shape[0] ({nW}) not divisible by n_experts ({n_experts})"
            )
        D = nW // n_experts
