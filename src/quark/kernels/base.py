...
def _format_problem_label(problem: dict) -> str:
    ...
    Used by :meth:`Kernel.load_tuned_config` when reading saved configs
    back. Underscores are used between key/value pairs and any key
    starting with ``_`` is skipped (those are scratch context like
    ``_torch_Q``).
    """
...
class Kernel(ABC):
    ...
    Subclasses MAY set:
        TARGET_NAME      — string used by the tuned-config JSON loader.
                           Set this so ``Kernel.load_tuned_config()``
                           works.
    """
    ...
    # tools/fuzz.py + tools/bench.py + autotune support.
    ...
    def tune_space_resolved(cls, spec, device) -> dict[str, list]:
        ...
        This method is what the autotune search walks — kernels
    ...
    # and the autotune search. The registry-aware contract overrides
